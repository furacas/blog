...
// 配置信息
const config = {
  username: 'beimengyeyu', // GitHub repository 所有者，可以是个人或者组织。对应Gitalk配置中的owner
  repo: 'beimengyeyu.github.io', // 储存评论issue的github仓库名，仅需要仓库名字即可。对应 Gitalk配置中的repo
  token: process.env.GITHUB_TOKEN, // 前面申请的 personal access token
  sitemap: path.join(__dirname, './public/sitemap.txt'), // 自己站点的 sitemap 文件地址
...
const sitemapXmlReader = (file) => {
  try {
    ...
    sitemap.root.children.forEach(function (url) {
      ...
      if(title.content.startsWith('标签 |') || title.content.startsWith('分类 |')){
        return false;
      }
